...
public function AccepterFormationGu($id){
    $Rq = "update lancement_formation set STATUT_DMD='Accepter' , DATE_REPONSE = (SELECT SYSDATE FROM DUAL) where id_dmd =".$id;
    $connexion = $this->getConnexion(); 
            $connexion->exec($Rq);
            $connexion->exec("commit");
}
public function RefuserFormationGu($id){
    $Rq = "update lancement_formation set STATUT_DMD='Non accepter' , DATE_REPONSE = (SELECT SYSDATE FROM DUAL) where id_dmd =".$id;
    $connexion = $this->getConnexion(); 
            $connexion->exec($Rq);
            $connexion->exec("commit");
}
