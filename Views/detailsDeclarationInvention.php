...
<div class="container-fluid">
  ...
  <?php if($_SESSION['type'] =='GuichetUnique' && $row['DECISION_FINALE'] == '' && $row['STATUT_RESP_GU'] == '' && $row['STATUT_DMD'] == 'En attente'){?>
                  <a href="index.php?action=transmettreInventionCir&&id_dmd=<?php echo $row['ID_DMD']?>" class="btn btn-success btn-icon-split" onclick="return confirm('Voulez-vous vraiment transmettre le dossier'); ">
                    <span class="icon text-white-50">
                      <i class="fas fa-check"></i>
                    </span>
                    <span class="text">Transmettre le dossier au directeur de la CIR</span>
                  </a><br><br>
                  <a href="index.php?action=RefuserDemandeGu&&id_dmd=<?php echo $row['ID_DMD'] ?>" class="btn btn-danger btn-icon-split" onclick="return confirm('Voulez-vous vraiment refuser le dossier'); ">
                    <span class="icon text-white-50">
  ...
  <?php if($row['DECISION_FINALE'] != '' && $_SESSION['type'] == 'GuichetUnique' && ($row['STATUT_DMD'] != 'Accepter' && $row['STATUT_DMD'] != 'Non accepter')){?>
                    <p style="font-size: 20px; ">La décision finale du CIR est : <strong style="color:green; "><?php echo $row['DECISION_FINALE'] ?> </strong></p>
                     <a href="index.php?action=AccepterInventionCir&&id_dmd=<?php echo $row['ID_DMD']?>" class="btn btn-success btn-icon-split" onclick="return confirm('Voulez-vous vraiment accepter le dossier'); ">
                    <span class="icon text-white-50">
                      <i class="fas fa-check"></i>
                    </span>
...
<?php if($_SESSION['type'] =='RespPoolCompetence' && $row['REPONSE_PC'] == ''  && $row['STATUT_DMD'] == 'En cours'){?>
  ...
  <?php if($_SESSION['type'] =='ResponsableCir' && $row['DECISION_FINALE'] == '' && $row['REPONSE_PC'] == '' && $row['STATUT_CIR'] == '' && $row['STATUT_DMD'] == 'En cours'){?>
                  <button type="Submit" class="button" onclick="return confirm('Voulez-vous vraiment transmettre le dossier'); "><span>Transmettre le dossier au reponsable de pool de competence</span></button>  
                   <br><br>
                  <a href="index.php?action=RefuserInventionCir&&id_dmd=<?php echo $row['ID_DMD']?>" class="btn btn-danger btn-icon-split" onclick="return confirm('Voulez-vous vraiment refuser le dossier'); ">
                    <span class="icon text-white-50">
                      <i class="fas fa-times"></i>
                    </span>
                    <span class="text">Refuser la demande</span>
